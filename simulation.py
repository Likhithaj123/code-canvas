...
def send_data_to_server(data):
    try:
        response = requests.post('http://localhost:5000/update_data', json=data)
...
class TrafficSignal:
    ...
    def serialize(self):
        return {
            ...
            'yellow': self.yellow,
            'green': self.green,
            'total_green_time': self.totalGreenTime
        }
...
def simulationTime():
    ...
    while(True):
        ...
        data = {
            ...
            'signals': [signal.serialize() for signal in signals],
            'vehicle_counts': [vehicles[direction]['crossed'] for direction in directionNumbers]
        }
